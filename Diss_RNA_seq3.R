...
# save this as the unnormalised data files
write.csv(df_new_size_row, file="GSE147352_PKM1_M2_unnormalised.csv", sep = '\t')
...
write.csv(normalised_df1, file="GSE147352_PKM1_M2_normalised.csv", sep = '\t')
...
#save file
write.csv(df_new_size_row2, file="GSE147352_PKLR_unnormalised.csv", sep = '\t')
...
#save file
write.csv(normalised_df2, file="GSE147352_PKLR_normalised.csv", sep = '\t')
...
#save file
write.csv(df_new_size_row3, file="GSE147352_PKM_transcript_unnormalised.csv", sep = '\t')
...
#save file
write.csv(normalised_df3, file="GSE147352_PKM_transcript_normalised.csv", sep = '\t')
...
#save file
write.csv(df_new_size_row4, file="GSE147352_PKM_all_unnormalised.csv", sep = '\t')
...
#save file
write.csv(normalised_df4, file="GSE147352_PKM_all_normalised.csv", sep = '\t')

#getting the tables of normalized readcounts for PFK transcripts (malcolms request)

#PFKL (ENSG00000141959) ENST00000349048.9
PFKL_exons <- c("ENSE00003843359", "ENSE00003556813", "ENSE00003468441",
                "ENSE00003468758", "ENSE00003620819", "ENSE00003487259",
                "ENSE00003482974", "ENSE00003610456", "ENSE00003668706", 
                "ENSE00003572167", "ENSE00003470814", "ENSE00003669897", 
                "ENSE00003682480", "ENSE00003688681", "ENSE00003543348", 
                "ENSE00003650066", "ENSE00003492099", "ENSE00003615543", 
                "ENSE00003538638", "ENSE00003533809", "ENSE00003458220", 
                "ENSE00003483524")

PFKL_exons_unique <- unique(PFKL_exons)
PFKL_exon_Index <- match(PFKL_exons_unique, rownames(exon_featurecounts))
PFKL_data <- exon_featurecounts[PFKL_exon_Index,]
PFKL_df <- as.data.frame(t(as.data.frame(PFKL_data)))
write.csv(PFKL_df, file="GSE147352_PFKL_all_exon_counts.csv", sep='\t')

#adding columns for sample type
PFKL_df_new <- cbind(dds_rnaseq2$Type, PFKL_df)
names(PFKL_df_new)
names(PFKL_df_new)[1] <- "SampleType"

#sorting these new dfs by sample type
PFKL_df_new <- PFKL_df_new[order(PFKL_df_new$SampleType),]
#saving new dfs has a file
write.csv(PFKL_df_new, file="GSE147352_new_PFKL_all.csv", sep = '\t')

#adding exon lengths row to the exon_read counts dfs
PFKL_exonswanted <- match(names(PFKL_df_new),exon_lengths$ExonID)
PFKL_exon_lengths <- exon_lengths[PFKL_exonswanted,]

# now r bind the size columns, eg put the sizes in the first row of current datafile
df_new_size_PFKL <- rbind(t(PFKL_exon_lengths)[2,], PFKL_df_new)
#save file
write.csv(df_new_size_PFKL, file="GSE147352_PFKL_all_unnormalised.csv", sep = '\t')

#Normalise the data!
PFKL_normalised <- as.matrix(df_new_size_PFKL)
xPFKL <- PFKL_normalised[,-1]
for(i in 1:ncol(xPFKL)){
  xPFKL[, i] <- (t(t(as.numeric(xPFKL[,i]))/as.numeric(xPFKL[1,i])))
}
normalised_PFKL_df <- as.data.frame(unlist(xPFKL))
# re add the sample type column
normalised_PFKL_df$SampleType <- df_new_size_PFKL$SampleType
dim(normalised_PFKL_df)

normalised_PFKL <- normalised_PFKL_df[c(23,1:22)]

#save file
write.csv(normalised_PFKL, file="GSE147352_PFKL_normalised.csv", sep = '\t')


#PFKM (ENSG00000152556) ENST00000359794.11
PFKM_exons <- c("ENSE00001650033", "ENSE00003548087", "ENSE00003692498", 
                "ENSE00003601910", "ENSE00003629396", "ENSE00003666513",
                "ENSE00003653751", "ENSE00003635953", "ENSE00003533607",
                "ENSE00003602515", "ENSE00003627820", "ENSE00003468832", 
                "ENSE00003489945", "ENSE00003518565", "ENSE00003597724", 
                "ENSE00003540367", "ENSE00003658387", "ENSE00003685157", 
                "ENSE00003624825", "ENSE00003499431", "ENSE00003478653", 
                "ENSE00003670446", "ENSE00001604638")

PFKM_exons_unique <- unique(PFKM_exons)
PFKM_exon_Index <- match(PFKM_exons_unique, rownames(exon_featurecounts))
PFKM_data <- exon_featurecounts[PFKM_exon_Index,]
PFKM_df <- as.data.frame(t(as.data.frame(PFKM_data)))
write.csv(PFKM_df, file="GSE147352_PFKM_all_exon_counts.csv", sep='\t')

#adding columns for sample type
PFKM_df_new <- cbind(dds_rnaseq2$Type, PFKM_df)
names(PFKM_df_new)
names(PFKM_df_new)[1] <- "SampleType"

#sorting these new dfs by sample type
PFKM_df_new <- PFKM_df_new[order(PFKM_df_new$SampleType),]
#saving new dfs has a file
write.csv(PFKM_df_new, file="GSE147352_new_PFKM_all.csv", sep = '\t')

#problem here
#adding exon lengths row to the exon_read counts dfs
# get unique rownames to get the correct index
#library("readxl")
#require(gdata)
#exon_info3 <- read.xls("Homo_sapiens.104.exon.excel.xls")
#exon_info3 <- read.excel("Homo_sapiens.104.exon.excel.xls")
#exon_info3 <- readxl::read_xls("Homo_sapiens.104.excel.xls")
#library(data.table)
#exon_info3 <- as.data.table(read_excel("Homo_sapiens.104.exon.excel.xls"))
exon_info_v2 <- read.table("Homo_sapiens.104.exon.excel.v2.xls",sep="\t")
exon_lengths_v2 <- subset(exon_info_v2, select = c(10,4))
names(exon_lengths_v2)[2] <- "exon_length"
names(exon_lengths_v2)[1] <- "ExonID"
PFKM_exonswanted <- match(names(PFKM_df_new),exon_lengths_v2$ExonID)
PFKM_exon_lengths <- exon_lengths_v2[PFKM_exonswanted,]

# now r bind the size columns, eg put the sizes in the first row of current datafile
df_new_size_PFKM <- rbind(t(PFKM_exon_lengths)[2,], PFKM_df_new)
#save file
write.csv(df_new_size_PFKM, file="GSE147352_PFKM_all_unnormalised.csv", sep = '\t')

#Normalise the data!
PFKM_normalised <- as.matrix(df_new_size_PFKM)
xPFKM <- PFKM_normalised[,-1]
for(i in 1:ncol(xPFKM)){
  xPFKM[, i] <- (t(t(as.numeric(xPFKM[,i]))/as.numeric(xPFKM[1,i])))
}
normalised_PFKM_df <- as.data.frame(unlist(xPFKM))

# re add the sample type column
normalised_PFKM_df$SampleType <- df_new_size_PFKM$SampleType
dim(normalised_PFKM_df)

normalised_PFKM <- normalised_PFKM_df[c(24,1:23)]
#save file
write.csv(normalised_PFKM, file="GSE147352_PFKM_normalised.csv", sep = '\t')

#PFKP (ENSG00000067057) ENST00000381125.9
PFKP_exons <- c("ENSE00001842048", "ENSE00003507485", "ENSE00003709765", 
                "ENSE00003785562", "ENSE00001784119", "ENSE00003475963", 
                "ENSE00003707565", "ENSE00003710369", "ENSE00003703345", 
                "ENSE00001638550", "ENSE00001754765", "ENSE00001791419", 
                "ENSE00001722279", "ENSE00001797079", "ENSE00001712413", 
                "ENSE00001643566", "ENSE00002182236", "ENSE00001634832", 
                "ENSE00001606866", "ENSE00001690153", "ENSE00001753049", 
                "ENSE00003722454")

PFKP_exons_unique <- unique(PFKP_exons)
PFKP_exon_Index <- match(PFKP_exons_unique, rownames(exon_featurecounts))
PFKP_data <- exon_featurecounts[PFKP_exon_Index,]
PFKP_df <- as.data.frame(t(as.data.frame(PFKP_data)))
write.csv(PFKP_df, file="GSE147352_PFKP_all_exon_counts.csv", sep='\t')

#adding columns for sample type
PFKP_df_new <- cbind(dds_rnaseq2$Type, PFKP_df)
names(PFKP_df_new)
names(PFKP_df_new)[1] <- "SampleType"

#sorting these new dfs by sample type
PFKP_df_new <- PFKP_df_new[order(PFKP_df_new$SampleType),]
#saving new dfs has a file
write.csv(PFKP_df_new, file="GSE147352_new_PFKP_all.csv", sep = '\t')

#adding exon lengths row to the exon_read counts dfs
PFKP_exonswanted <- match(names(PFKP_df_new),exon_lengths$ExonID)
PFKP_exon_lengths <- exon_lengths[PFKP_exonswanted,]

# now r bind the size columns, eg put the sizes in the first row of current datafile
df_new_size_PFKP <- rbind(t(PFKP_exon_lengths)[2,], PFKP_df_new)
#save file
write.csv(df_new_size_PFKP, file="GSE147352_PFKP_all_unnormalised.csv", sep = '\t')

#Normalise the data!
PFKP_normalised <- as.matrix(df_new_size_PFKP)
xPFKP <- PFKP_normalised[,-1]
for(i in 1:ncol(xPFKP)){
  xPFKP[, i] <- (t(t(as.numeric(xPFKP[,i]))/as.numeric(xPFKP[1,i])))
}
normalised_PFKP_df <- as.data.frame(unlist(xPFKP))
# re add the sample type column
normalised_PFKP_df$SampleType <- df_new_size_PFKP$SampleType
dim(normalised_PFKP_df)

normalised_PFKP <- normalised_PFKP_df[c(23,1:22)]
#save file
write.csv(normalised_PFKP, file="GSE147352_PFKP_normalised.csv", sep = '\t')

#end of getting PFK transcript exon reads.
